...
也可以在游戏内直接用 `/chatorder enable` / `/chatorder disable` 指令开关该功能。该指令只修改当前客户端的本地配置：单人游戏和局域网房主可以直接使用；连接独立服务器时，客户端需要在本地开启，服主也必须在服务器的配置文件中开启，服务器关闭时不会接受任何客户端的弹幕订单。
...
| `随机订单` | 从完整菜单中随机生成订单 |
| `外带`、`外卖`、`带走`、`打包` | 将订单设为外卖 |
| `堂食` | 强制生成堂食订单，优先级高于外卖关键词 |

如果发言中同时包含“堂食”和外卖关键词，将按堂食处理；两类关键词都没有时，由原有订单逻辑随机决定堂食或外卖。
