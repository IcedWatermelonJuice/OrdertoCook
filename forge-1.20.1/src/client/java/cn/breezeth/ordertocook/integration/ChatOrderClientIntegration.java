...
/** 将 Forge 原生消息事件接入弹幕点单公共解析器。 */
public final class ChatOrderClientIntegration {
    private ChatOrderClientIntegration() {}

    public static void register() {
        ChatOrderClientApi.initialize();
        MinecraftForge.EVENT_BUS.addListener(ChatOrderClientIntegration::onSystemChat);
        MinecraftForge.EVENT_BUS.addListener(ChatOrderClientIntegration::onPlayerChat);
    }
    ...
    private static void onSystemChat(ClientChatReceivedEvent.System event) {
        if (!event.isOverlay() && ConfigManager.get().chatOrderCaptureNativeMessages) {
            ChatOrderClientApi.submitMessage("forge_system", extractMatchableMessage(event.getMessage()));
        }
    }
